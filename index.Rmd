...
At the benchmark of only processing a small number of processes, solution 2 may outperform `Rcpp` solution. Nevertheless, the latter *scales* considerably greater. Let's further examine this point. (It all depends on your machine. For example on my MacBook 12" solution 2 is faster overall at 2 processes but on my MacBook Pro 13" the `Rcpp` solution constantly outperform the others.) 
...
Well, the result suggests that `Rcpp` solution performs fairly equal to the `data.table` solution. Indeed the former is a little bit slower in our particular trial run. Time to inspect the scalability issue.
