...
hexdump <- system(sprintf("xxd -g1 %s | cut -d \" \" -f 2-17", samplefile), intern=TRUE) # unix only
hexdump <- unlist(strsplit(paste(hexdump, collapse=' '), ' '))
...
Or we can use some external library about text mining.
...
But the performance of the above two solutions are both not satisfying when it comes to large-scale problem.

Now let's try `Rcpp` to see what we could further achieve.

```{r ex1:cpp_solution, cache=TRUE}
# third trial: use Rcpp to write the function in C++
library(Rcpp)
cppFunction('
